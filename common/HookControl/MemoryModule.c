...
HMEMORYMODULE MemoryLoadLibrary(const void *data,BOOL bIsRun)
{
	return MemoryLoadLibraryEx(data, NULL, bIsRun, _LoadLibrary, _GetProcAddress, _FreeLibrary, NULL);
}

unsigned char * MemoryLoadLibraryToAddr(const void *data, unsigned char * uBaseAddr,HMEMORYMODULE * hMemModule)
{
	*hMemModule = MemoryLoadLibraryEx(data, uBaseAddr, FALSE, _LoadLibrary, _GetProcAddress, _FreeLibrary, NULL);
	return ((PMEMORYMODULE)*hMemModule)->codeBase;
}

HMEMORYMODULE MemoryLoadLibraryEx(const void *data, unsigned char * uBaseAddr, BOOL bIsRun,
    CustomLoadLibraryFunc loadLibrary,
    CustomGetProcAddressFunc getProcAddress,
    CustomFreeLibraryFunc freeLibrary,
    void *userdata)
{
	...
	locationDelta = (SIZE_T)((unsigned long)(0 == uBaseAddr) ? code : uBaseAddr - old_header->OptionalHeader.ImageBase);
    if (locationDelta != 0) {
	...
	if (bIsRun && result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
...
FARPROC MemoryGetProcAddress(HMEMORYMODULE module, LPCSTR name)
{
	...
	codeBase = ((PMEMORYMODULE)module)->codeBase;
	directory = GET_HEADER_DICTIONARY((PMEMORYMODULE)module, IMAGE_DIRECTORY_ENTRY_EXPORT);
