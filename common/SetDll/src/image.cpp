...
public:                                                 // File Functions
    BOOL                    Read(HANDLE hFile, const char * pszSectionName);
    BOOL                    Write(HANDLE hFile, const char * pszSectionName);
    BOOL                    Close();
...
BOOL CImage::Read(HANDLE hFile, const char * pszSectionName)
{
    DWORD n;
    PBYTE pbData = NULL;
    DWORD cbData = 0;
    ...
    for (n = 0; n < m_NtHeader.FileHeader.NumberOfSections; n++) {
        if (strcmp((PCHAR)m_SectionHeaders[n].Name, pszSectionName) == 0) {
...
BOOL CImage::Write(HANDLE hFile,const char * pszSectionName)
{
    DWORD cbDone;
    ...
    if (fNeedDetourSection || !m_pImageData->IsEmpty()) {
        ...
        HRESULT hrRet = StringCchCopyA((PCHAR)m_SectionHeaders[nSection].Name, IMAGE_SIZEOF_SHORT_NAME , pszSectionName);
        if (FAILED(hrRet))
            return FALSE;
...
//////////////////////////////////////////////////////////////////////////////
//
PDETOUR_BINARY WINAPI DetourBinaryOpen(HANDLE hFile, const char * pszSectionName/* = ".ssoor"*/)
{
    ...
    if (!pImage->Read(hFile, pszSectionName)) {
        delete pImage;
        return FALSE;
    }
...
BOOL WINAPI DetourBinaryWrite(PDETOUR_BINARY pdi, HANDLE hFile , const char * pszSectionName/* = ".ssoor"*/)
{
    ...
    return pImage->Write(hFile, pszSectionName);
}
